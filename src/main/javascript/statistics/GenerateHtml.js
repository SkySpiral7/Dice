...
{
   //TODO: rename secondColumn to comparison and add docs
   ...
   secondColumn = secondColumn.replace(/^([!=])=*/, '$1==');  //forces !== and ===
   ...
   out += '<th>Chance</th>';
   //TODO: could display &ne; for ≠ but then I'd have to allow that as input as well
   if('===' !== secondColumn) out += '<th>' + secondColumn.replace('>', '&gt;').replace('<', '&lt;').replace(/=+/, '=') + '</th>';
   ...
   for (var i = 0; i < stats.length; ++i)
   {
      ...
      if('===' !== secondColumn) out += '<td align="right" width="1%">' + (100 * secondValues[i]).toFixed(3) + '%</td>';
